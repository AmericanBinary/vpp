...
func (v *Validator) validateRemoteNodeRoutes(node *telemetrymodel.Node, vrfMap VrfMap, routeMap RouteMap) int {
	...
	_, err := findInterface(ipnet.DefaultVxlanBVIInterfaceName, node.NodeInterfaces)
	if err != nil {
		numErrs++
	...
	for _, othNode := range nodeList {
		...
		ifc, err := findInterface(ipnet.DefaultVxlanBVIInterfaceName, othNode.NodeInterfaces)
		if err != nil {
			numErrs++
		...
		if err == nil {
			...
			numErrs += v.validateRoute(othNode.NodeIPam.VppHostNetwork, 1, vrfMap, routeMap, node.Name,
				ipnet.DefaultVxlanBVIInterfaceName, bviAddr, 0, vpp_l3.Route_INTRA_VRF)
...
func (v *Validator) validateRouteToLocalVxlanBVI(node *telemetrymodel.Node, vrfMap map[uint32]Vrf,
	...
	numErrs := 0
	loopIf, err := findInterface(ipnet.DefaultVxlanBVIInterfaceName, node.NodeInterfaces)
	if err != nil {
		numErrs++
		v.Report.AppendToNodeReport(node.Name, err.Error())
		return numErrs
